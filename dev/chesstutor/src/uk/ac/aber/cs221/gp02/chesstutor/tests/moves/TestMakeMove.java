/*
 * @(#) TestMakeMove.java 0.5 2023/05/03
 *
...
/**
 * Unit tests for the MakeMove class. These tests will try to move pieces to empty spaces, capture
 * pieces and then try to move a piece off the board
 *
 * @author Lance [lvs1]
 * @author Thomas [tpr3]
...
public class TestMakeMove {
   ...
   void movePieceToEmptySpace() {
      ...
      assertFalse(movedPiece[6][1].hasPiece()); //Checks if previous space is empty
   ...
   void takeOpponentsPiece(){
      ...
      assertFalse(movedPiece[4][1].hasPiece());
