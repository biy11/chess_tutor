...
public class CheckChecker {
    ...
    */
   public static List<int[]> checkChecker(Board board, int x, int y){
      Square[][] boardArray = board.getBoardArray();

      Color attackingColor = boardArray[x][y].getPiece().getEnemyPieceColor();
   ...
   public static boolean checkMateChecker(Board board, int x, int y){
      ...
      List<int[]> attackers = checkChecker(simBoard, x, y);
      ...
      if(attackers.size() >= 2){
         ...
         for(int[] currentMove:possibleKingMoves){
            ...
            if(checkChecker(simBoard, currentMove[0], currentMove[1]).isEmpty()){
      ...
      //Otherwise if the king is in check and not double check
         ...
         for(int i = 0; i < 8; i++){
            for(int j = 0; j < 8; j++){
               ...
               if(!currentSquare.isHasPiece()) continue;
               ...
               for(int[] newMove:moves){
                  ...
                  if(currentSquare.getPiece().getPieceType() == Type.KING){
                     newAttackers = checkChecker(simBoard, newMoveX, newMoveY);
                  }

                  //Otherwise get the attackers of the square if the king has not moved
                  else{
                     newAttackers = checkChecker(simBoard, x, y);
                  }
