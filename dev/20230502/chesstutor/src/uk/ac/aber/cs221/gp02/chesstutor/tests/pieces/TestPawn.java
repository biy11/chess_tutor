...
public class TestPawn {
   ...
   private void printValidMoves(Board board, int x, int y){ //for debugging only
      ...
      for ( int i=0 ; i<obj[y][x].getPiece().getPossibleMoves(board, obj[y][x].getPiece(), x, y).size();i++) {
         for(int j=0 ; j < 2;j++) {
            System.out.println(j + ":" + obj[y][x].getPiece().getPossibleMoves(board, obj[y][x].getPiece(), x, y).get(i)[j]);
         }
      }
   }
